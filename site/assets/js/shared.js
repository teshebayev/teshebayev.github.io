 ...
 * Returns [{ group: {id, label, count}, articles: [...] }, ...]
 * Articles without tags go under a synthetic "other" group.
 * Same shape as applyReadingOrder() so the same renderer handles both.
 */
export function groupByPrimaryTag(manifest, articles) {
  ...
  return Array.from(groups.entries())
    .map(([id, list]) => ({
      group: { id, label: id === "_other" ? "Прочее" : tagLabel(manifest, id), count: list.length },
      articles: list,
    }))
...
// =========================================================================
//  Reading order — manual curriculum support
// =========================================================================

/**
 * Does this category have a curated reading order?
 */
...
/**
 * Apply a category's curated `reading_order` to its articles.
 *
 * Returns [{ group: {id, label, count}, articles: [...] }, ...] — same shape
 * as groupByPrimaryTag(), so the same renderer works for both modes.
 *
 * Articles not listed in any group land in a final "Прочее" group, sorted by
 * date (newest first). Slugs that are listed but don't exist are silently
 * ignored — won't break the page.
 */
export function applyReadingOrder(manifest, category) {
  ...
  for (const g of category.reading_order || []) {
    ...
    groups.push({
      group: { id: g.group || g.label, label: g.label, count: articles.length },
      articles,
    });
  }
  ...
  if (leftover.length > 0) {
    groups.push({
      group: { id: "_rest", label: "Прочее", count: leftover.length },
      articles: leftover, // already date-sorted by articlesInCategory()
    });
  }
  return groups;
}
...
/**
 * Flatten reading-order groups into a single sequence, preserving order.
 * Used to compute prev/next that follows the curriculum.
 */
export function readingOrderSequence(manifest, category) {

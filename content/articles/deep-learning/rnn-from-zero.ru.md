...
<div class="callout">
  ...
  шагам: кросс-энтропия считается на каждом шаге отдельно, и результаты
  складываются. Особенность RNN появляется только в том, как градиент этой суммы
  проходит назад сквозь время.
</div>
...
<div class="callout">
  <strong>Главная мысль части:</strong> <code>W<sub>hh</sub></code> квадратная,
  <code>[H × H]</code>, потому что состояние на каждом шаге должно возвращаться в

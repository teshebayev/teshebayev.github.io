  ...
  target.innerHTML = `
    <aside class="sidebar">
      ...
      <div class="sidebar__social">
        ...
        ${site.social.linkedin ? socialIcon(site.social.linkedin, "linkedin") : ""}
      ...
      <a href="assets/cv/cv.pdf" download class="sidebar__cv">${ui(manifest, "download_cv", lang)}</a>
...
function socialIcon(href, kind) {
  ...
  const resolvedHref = kind === "mail" ? `mailto:${href}` : href;
  const target = kind === "mail" ? "" : ` target="_blank" rel="noopener"`;
  return `<a href="${resolvedHref}"${target} aria-label="${kind}">${icons[kind]}</a>`;
}
